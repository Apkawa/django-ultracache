...
def drf_cache(func, backend=None, timeout=300):
    ...
    def wrapped(context, request, *args, **kwargs):
        ...
        if do_cache:
            ...
            if evaluate is not None:
                ...
                if callable(evaluate):
                    param = evaluate(context, request)
                li.append(param)

            if "django.contrib.sites" in settings.INSTALLED_APPS:
                li.append(get_current_site_pk(request))

            cache_key = md5(":".join([str(l) for l in li])).hexdigest()
        ...
        if do_cache:
            ...
            response.render()
            timeout = viewset_settings.get("timeout", default_timeout)
